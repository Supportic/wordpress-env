...
{
    wp_die(
        __('No feed available, please visit the', 'local-wpdev') . '<a href="' . esc_url(home_url('/')) . '">' .
            __('homepage', 'local-wpdev') . '</a>!'
    );
}
// Disable RSS Feed.
...
if (!function_exists('wpdev_dequeue_jquery_migrate')) {
    function wpdev_dequeue_jquery_migrate(WP_Scripts $scripts)
    {
        if (isset($scripts->registered['jquery']) && !empty($scripts->registered['jquery'])) {
